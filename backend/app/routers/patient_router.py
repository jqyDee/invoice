...
from ..schemas import PaginatedPatients, Patient, PatientCreate
...
def get_patients(
    ...
    total: int | None = db.scalar(select(func.count()).select_from(statement.subquery()))
...
def create_patient(patient_new: PatientCreate, db: Session = Depends(get_db)):
