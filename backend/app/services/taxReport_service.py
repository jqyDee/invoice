...
def get_tax_report_csv(year: int, db: Session) -> str:
    ...
    with output:
        ...
        writer.writerow([
            "Rechnungsnummer", "Rechnungstyp", "Rechnungsdatum", "Bezahlt am",
            "Km bei Abrechnung", "Gesamte Km",
            "Behandlungstermine", "Rechnungsbetrag"
        ])
        for row in rows:
            writer.writerow([
                row.invoice_number, row.invoice_type, row.invoice_date, row.paid_date,
                row.kilometers_at_billing, row.total_kilometers_travelled,
                row.number_of_treatment_dates, row.invoice_total,
            ])
