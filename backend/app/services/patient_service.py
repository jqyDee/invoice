...
from ..models import PatientDB, InvoiceDB
...
def perform_delete_patient(
        patient_id: int,
        db: Session
) -> PatientDB:
    existing_invoice = db.scalars(
        select(InvoiceDB).where(InvoiceDB.patient_id == patient_id)
    ).first()
